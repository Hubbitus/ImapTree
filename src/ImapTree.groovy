...
cli.c(longOpt: 'cached', 'run from cached file. No information gathered from imap account actually. Instead read previously saved file config.cache.xml.fullXmlCache. Useful for deep analysis and experiments.', required: false)
...
if(opt.h /*|| opt.arguments().isEmpty()*/ ) {
	...
	File xmlCacheFile = new File((String)GlobalConf.cache.xml.fullXmlCache.replace('%{account}', accountName));

	if (!opt.c){
		imapTree = new ImapTreeSize(imapAccount);
		if (GlobalConf.cache.xml.fullXmlCache){
